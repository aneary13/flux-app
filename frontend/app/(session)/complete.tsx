...
export default function CompleteSessionScreen() {
  ...
  const handleFinishSession = async () => {
    // 1. Safety Guard: If we don't have a session ID or the anchor pattern metadata,
    // we cannot update the backend state correctly.
    const anchorPattern = sessionData?.metadata.anchor_pattern;

    if (!sessionId || !anchorPattern) {
      console.warn('Missing session metadata, skipping backend update.');
      clearSession();
      router.replace('/');
      return;
    }
    ...
    try {
      ...
      // 3. Build the payload. TypeScript now knows anchorPattern is strictly 'MovementPattern'
      // because of our guard clause above.
      const payload = {
