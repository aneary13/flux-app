...
export default function ActiveSessionScreen() {
  ...
  const onViewableItemsChanged = useRef(({ viewableItems }: { viewableItems: ViewToken[] }) => {
    if (viewableItems.length > 0) {
      ...
      // We only process if the index is valid and has actually changed
      if (newIndex !== null && newIndex !== previousIndex.current) {
        ...
        if (blocks && blocks[previousIndex.current]) {
          ...
          // Atomic accumulation of block duration
          state.addBlockTime(prevBlockType, timeSpent);
        }

        // Reset local clock for the new active block
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <Typography variant="body" style={styles.pageIndicator}>
