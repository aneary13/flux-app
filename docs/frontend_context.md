...
1.  **Home/Dashboard:** Displays the user's "Pattern Readiness" (e.g., "Fully Primed", "Fatigued") derived entirely from the backend View Model, and overall readiness to start. In time, this screen will have multiple tabs (e.g Home, Settings, Progress, History, etc.)
...
* **Thin Client:** The frontend performs NO biological calculations, nor any datetime math. If a value needs to be rounded, a progression level determined, or a date elapsed time calculated (e.g., "days since last trained"), it must come pre-calculated from the backend View Model.
